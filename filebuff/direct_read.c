...
main(int argc, char *argv[])
{
	 ...
	 * 2 * 'alighment' and then adding 'alignment' to the returned pointer,
	 * we ensure that 'buf' is aligned on a non-power-of-two multiple of
	 * 'alignment'. We do this to ensure that if, for example, we ask 
	 * for a 256-byte aligned buffer, we don't accidentally get a buffer 
	 * that is also aligend on a 512-byte boundary */
