...
userIdFromName(const char *name)
{
	...
	u = strtol(name, &endptr, 10);		/* As a convenience to the caller */
...
gid_t		/* Return GID corrresponding to 'name', or -1 on error */
groupIdFromName(const char *name)
{
	...
	g = strtol(name, &endptr, 10);		/* As a convenience to the caller */
	if (*endptr == '\0')			/* allow a numeric string */
		return g;
